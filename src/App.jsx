...
function App() {
  ...
  return (
    <section className={`Page-Wrapper relative`}>
      <Navbar />
      <Sidebar />
      <Cart />
      ...
      <div className="h-12 nav-bottom flex justify-between md:hidden fixed bottom-0 z-20 shadow-lg bg-white w-full px-3">
        ...
        >
          ...
          {cart.length > 0 ? (
            <span class="absolute top-0 right-0 mt-0.5 rounded-full py-1 px-1.5 text-[10px] font-semibold leading-none bg-customgreen text-white">
              {cart.length}
            </span>
          ) : (
            ""
